...
void usage(char *cmd)
{
	fprintf(stdout,"The Restore Factory Default restore the factory default setting on local/remote device\n");
	fprintf(stdout,"\n");
...
int main(int argc, char* argv[])
{
	...
	if (CMDHelpers_Restore_Factory_Default(bHasDeviceIP,strDeviceIP,bHasAdapterIP,strAdapterIP,bHasDeviceMac,strDeviceMAC,&restoreFactoryDefault) == FALSE)
	{
		return 1;
	}

	CMDHelpers_Print_Connection_Information(restoreFactoryDefault.Connection);

	printf("Restore Factory Default\t\t\t");
	Printf_Highlight("[OK]\n");
