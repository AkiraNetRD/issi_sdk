...
void usage(char *cmd)
{
	fprintf(stdout,"The Device Information tool shows information about the local device and remote devices in the same domain.\n");
	fprintf(stdout,"\n");
	...
	fprintf(stdout,"-L       - Query only the Local device\n");
	fprintf(stdout,"\n");
...
int main(int argc, char* argv[])
{
	...
	bool						QueryOnlyLocalDevice = FALSE;
	bool						bAdvanced = FALSE;

	while ((c = getopt(argc,argv,"m:i:I:L?hVX")) >= 0) 
	{
		switch(c)
		{
		case '?':
	...
	if (CMDHelpers_GetStation(bHasDeviceIP,strDeviceIP,bHasAdapterIP,strAdapterIP,bHasDeviceMac,strDeviceMAC,FALSE,QueryOnlyLocalDevice,&getstation) == FALSE)
	{
		return 1;
	}

	if (QueryOnlyLocalDevice)
	{
		if (getstation.Size > 1)
		{
			getstation.Size = 1;
		}
	}

	Print_Device_Information(&getstation, bAdvanced);
