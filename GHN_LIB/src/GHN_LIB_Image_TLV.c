...
BOOL Append_TLV_Section(BIN_file_t* BIN_file, eBIN_Section Section, UINT32 Length)
{
	...
	BIN_section_header.CheckSum = 0xFFFFFFFF; //CalculatePacketChecksum((UINT8*)Buffer, BIN_section_header.Length);
							...
							char*			Delete_ParameterName)
{
	...
	{
		StartIndex_Section=0;
		StartIndex_FirstValue=0;
		Size = SectionSize;
	}
	else if (Section == eBIN_Section_Modified_Parameters_Block)
	{
		if (SectionBuffer[0] == 0x01)
		{
			StartIndex_Section=0;
			StartIndex_FirstValue=1;
		}
		else if (SectionBuffer[SectionSize/2] == 0x01)
		{
			StartIndex_Section=SectionSize/2;
			StartIndex_FirstValue=SectionSize/2+1;
		}
		else
		{
			return FALSE;
	...
	SectionBuffer_Temp = (UINT8*)malloc(Size);
	...
	memset(SectionBuffer_Temp, 0xFF, Size);

	Index1=StartIndex_FirstValue;
	Index2=0;

	if (Section == eBIN_Section_Modified_Parameters_Block)
	{
		// Set the Active-Bit
		SectionBuffer_Temp[0] = 0x01;
		Index2++;
	}
	
	while ((SectionBuffer[Index1]) != 0xFF)
	{
		if (strlen((char*)&SectionBuffer[Index1]) > 256)
		{
	...
	memcpy(&SectionBuffer[StartIndex_Section], &SectionBuffer_Temp[0], Size);
						...
						void*			New_ParameterValue)
{
	...
	{
		if (strlen((char*)&SectionBuffer[Index]) > 256)
		{
			printf("TLV format is broken!\n");
			break;
		}
...
BOOL Get_TLV_Parameter_Boolean_Auto(BIN_file_t*		BIN_file,
									...
									UINT8*			Value)
{
	if (Get_TLV_Parameter_Boolean(BIN_file, eBIN_Section_Modified_Parameters_Block, ParameterName, Value) == TRUE)
	{
		return TRUE;
	}
...
BOOL Get_TLV_Parameter_UINT8_Auto(	BIN_file_t*		BIN_file,
									...
									UINT8*			Value)
{
	if (Get_TLV_Parameter_UINT8(BIN_file, eBIN_Section_Modified_Parameters_Block, ParameterName, Value) == TRUE)
	{
		return TRUE;
	}
...
BOOL Get_TLV_Parameter_UINT16_Auto(	BIN_file_t*		BIN_file,
									...
									UINT16*			Value)
{
	if (Get_TLV_Parameter_UINT16(BIN_file, eBIN_Section_Modified_Parameters_Block, ParameterName, Value) == TRUE)
	{
		return TRUE;
	}
...
BOOL Get_TLV_Parameter_UINT32_Auto(	BIN_file_t*		BIN_file,
									...
									UINT32*			Value)
{
	if (Get_TLV_Parameter_UINT32(BIN_file, eBIN_Section_Modified_Parameters_Block, ParameterName, Value) == TRUE)
	{
		return TRUE;
	}
...
BOOL Get_TLV_Parameter_String_Auto(	BIN_file_t*		BIN_file,
									...
									char*			Value)
{
	if (Get_TLV_Parameter_String(BIN_file, eBIN_Section_Modified_Parameters_Block, ParameterName, Value) == TRUE)
	{
		return TRUE;
	}
...
BOOL Get_TLV_Parameter_Array_Auto(	BIN_file_t*		BIN_file,
									...
									UINT8*			Value)
{
	if (Get_TLV_Parameter_Array(BIN_file, eBIN_Section_Modified_Parameters_Block, ParameterName, ParameterLength, Value) == TRUE)
	{
		return TRUE;
	}
							...
							UINT8			Configuration2)
{
	...
	if (GetSection(BIN_file, eBIN_Section_Modified_Parameters_Block, FALSE, &SectionBuffer, &SectionSize) == FALSE)
	{
		return FALSE;
	}

	SectionBuffer[0] = Configuration1;
	SectionBuffer[SectionSize/2-1] = Configuration2;
